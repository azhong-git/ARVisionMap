...
{   
	...
	double dy = 0.0;
	double dz = 0.0;
	static float [] rotateX = {-90, 0, 0 };
	static float [] rotateZ = {0, 0, 0};
	static float [] zoom = {1, 1, 1};
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, final float distanceX,
		...
		if (currentMode == modes.modeVisitor.ordinal()
				&& currentDevice == devices.Afinia.ordinal()) {
			if (Math.abs(distanceX) > Math.abs(distanceY))
				rotateZ[currentPrototypeAfinia] = (rotateZ[currentPrototypeAfinia] - distanceX) % 360;
			else
				rotateX[currentPrototypeAfinia] = (rotateX[currentPrototypeAfinia] - distanceY) % 360;
		}
		return true;
	}
	...
	public boolean onScale(ScaleGestureDetector detector) {
		...
		if (currentMode == modes.modeVisitor.ordinal()
				&& currentDevice == devices.Afinia.ordinal()) {
			zoom[currentPrototypeAfinia] = 1/detector.getScaleFactor();
		}
		return false;
	}
